...
## H. 3D 预览：恢复原工作室环境光（2026-09-18）

按老板最新要求：**纯白 `#ffffff` 视口 + 灰白白模 + 原工作室环境光**。保留环境贴图与接触阴影，仅恢复工作室灯光配置。
...
默认 `AmbientLight + DirectionalLight` 方案已撤回；场景自带 `background/lights/ground` 和对象材质方案仍保留在历史提交 `ca4cd1a` 中。
