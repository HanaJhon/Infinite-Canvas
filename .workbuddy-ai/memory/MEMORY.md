...
- **视觉风格（2026-09-18 当前）**：**纯白 `#ffffff` 视口 + 灰白石膏白模 + 工作室环境光**；查看器固定 PMREM 摄影棚环境贴图、半球光、主/辅/轮廓三点方向光和白色背景，所有图元统一使用灰白 `#d9dce1` 材质，地面使用 `ShadowMaterial` 接收柔和接触阴影。场景 JSON 只保留几何与相机，模型输出的颜色/灯光/地面字段不再影响视觉；FOV 滑块与相机姿态持久化保留。
