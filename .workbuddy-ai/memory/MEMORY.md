...
- **视觉风格（2026-09-18 当前）**：**纯白 `#ffffff` 视口 + 默认 `AmbientLight`/`DirectionalLight` + 场景对象颜色**；查看器固定白色地面与浅灰网格，旧场景的深色 `background`/`ground.color` 不再污染视口。缺少任一灯光类型时自动补齐默认灯光；不使用半球光、三点工作室布光、PMREM 环境贴图或 `ShadowMaterial` 接触阴影。场景 JSON 仍保留 `background/lights/ground` 与对象 `color/roughness/metalness`，但查看器对白色背景/地面和默认灯光有最终兜底。
