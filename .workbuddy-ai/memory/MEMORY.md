...
- **视觉风格（2026-09-18 当前）**：**纯白 `#ffffff` 视口 + 场景自带颜色/灯光/地面/网格**；默认两盏灯为 `AmbientLight` + `DirectionalLight`，不使用半球光、三点工作室布光、PMREM 环境贴图或 `ShadowMaterial` 接触阴影。场景 JSON 保留 `background/lights/ground` 与对象 `color/roughness/metalness`。
