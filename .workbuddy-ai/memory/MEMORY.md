...
- 🚨 **远端 4 个旧 Release 资产 = 比 git 历史更大的暴露面**（公开可下、合计 **3.41 GB**，两个内含**整个 `.git`**、四个都含**作者画布 + `history.json`**）。**已拍板全部删掉** → 用 **`output/发布-1.1.1-一键.sh`**（第 0 步先校验 sha256 + 跑安全审计，再推提交、删旧资产、发 1.1.1、自检两个 URL）。
- 📌 **查远端 Release 不必消耗 API 配额**：用 `releases/download/<tag>/<asset>` 的 **HEAD** 量体积、**Range** 读 zip 中央目录。⚠️ **拉真实资产名**用 `releases/expanded_assets/<tag>` 的 HTML（**别猜文件名** —— 猜错会得 404 误判「已删除」）。⚠️ **ZIP 中央目录偏移**：压缩方式 **10**、压缩大小 **20**、未压缩大小 **24**、本地头偏移 **42**。
- ⚠️ `git cat-file --batch` 要处理 `<sha> missing`（2 字段）头（写成 `!= 3: break` 会**静默截断后续对象**）；`SENSITIVE` 里 `AUTH` 要写 `(?<!o)AUTH`。
- ✅ **`.workbuddy-ai/memory/` 已取消 git 跟踪**（2026-09-20，提交 `b098299`）：6 个文件含内部推理与路径、已实际带进过密钥 → 加 `.gitignore` 条目 + `git rm --cached`（**index-only，磁盘文件不动**，快照比对 6/6 完好）。**要恢复共享：`git add -f .workbuddy-ai/memory/`**。⚠️ 本目录是项目数据不是缓存，**不要删**。
