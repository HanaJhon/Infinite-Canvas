...
def is_grsai_nano_provider(provider, model):
    if str(model or "").strip().lower() != "nano-banana-pro":
        return False
    ...
    try:
        ...
        host = ""
    return name == "grsai" or "grsai" in host

def grsai_image_size(resolution, size):
    requested = str(resolution or "").strip().lower()
    if requested in {"1k", "2k", "4k"}:
        return requested.upper()
    inferred = runninghub_resolution_from_size(size, "1k")
    return inferred.upper() if inferred in {"1k", "2k", "4k"} else "1K"

def grsai_aspect_ratio(aspect_ratio, size):
    value = str(aspect_ratio or "").strip().lower().replace(" ", "")
    if value == "auto":
        return "auto"
    return value if re.fullmatch(r"\d+:\d+", value) else runninghub_aspect_from_size(size)
...
async def generate_ai_image(prompt, size, quality, model, reference_images=None, provider_id="comfly", aspect_ratio="", resolution=""):
    ...
    is_grsai_nano = is_grsai_nano_provider(provider, model)
    request_timeout = httpx.Timeout(connect=20.0, read=1800.0, write=120.0, pool=20.0) if (is_gpt2 or is_apimart or is_grsai_nano or image_request_mode in {"openai-json", "openai-video-proxy", "openai-responses"}) else AI_REQUEST_TIMEOUT
    async with httpx.AsyncClient(timeout=request_timeout) as client:
        ...
        if is_grsai_nano:
            _grsai_refs = image_refs[:ONLINE_IMAGE_REFERENCE_MAX]
            body = {
                "model": model,
                "prompt": prompt,
                "images": [reference_to_data_url(ref, max_size=ref_image_max_size(len(_grsai_refs))) for ref in _grsai_refs],
                "aspectRatio": grsai_aspect_ratio(aspect_ratio, size),
                "imageSize": grsai_image_size(resolution, size),
                "replyType": "json",
            }
            grsai_url = provider_endpoint_url(provider, "image_generation_endpoint", "/v1/api/generate")
